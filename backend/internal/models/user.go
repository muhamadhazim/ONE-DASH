...
type User struct {
	...
	Email        string    `gorm:"not null;size:255;unique" json:"email"`   // UNIQUE constraint exists in DB
	Username     string    `gorm:"not null;size:50;unique" json:"username"` // UNIQUE constraint exists in DB
